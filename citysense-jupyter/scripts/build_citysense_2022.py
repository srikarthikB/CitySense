"""
CitySense - 2022 data-preparation pipeline.

Builds the complete 946-grid x 365-date x 24-hour universe for 2022,
...
from the end of 2021 (which itself already carries the end-of-2020
state) rather than resetting to zero on 2022-01-01.
...
For 2021 -> 2022, the carry-over constant per group must be the FULL
crime total accumulated over BOTH 2020 and 2021 for that group (2021's
own crime_count column only reflects 2021 activity -- it does NOT
include the 2020 contribution baked into 2021's historical_* columns).
Because raw crime_count sums are simple additive quantities untouched
by any cumsum/shift bookkeeping, this is computed directly as:

    carry_2022[group] = sum(crime_count where year==2020, group)
                       + sum(crime_count where year==2021, group)

This is spot-checked below against the alternative derivation "last
chronological 2021 row's historical_X + that row's own crime_count"
for the grid-level key, to confirm the two approaches agree.
...
For each 2022 batch (a subset of grids), the within-2022 cumsum().shift(1)
is computed exactly as in the 2020/2021 scripts (so no future 2022
information leaks backward), and then the matching carry-over constant
is added to every row of that group. The very first 2022 row of a group
therefore starts from its 2021 ending total (which already reflects
2020 + 2021), and no in-memory full-year 2022 frame, or full 2020-2025
frame, is ever built.
"""
...
OUT_PATH = "/home/claude/work/citysense_2022.parquet"
YEAR = 2022
GRID_BATCH_SIZE = 100
...
#    established in citysense_2021.parquet (itself inherited unchanged from
#    citysense_2020.parquet) -- do not rederive a new grid system.
# ---------------------------------------------------------------------------
grid_lookup = (
    pd.read_parquet(PATH_2021, columns=["grid_id", "lat_grid", "lon_grid"])
    .astype({"grid_id": "str"})
...
print(f"[1] Grids reused from citysense_2021.parquet: {N_GRIDS}")
...
# ---------------------------------------------------------------------------
# 2) Carry-over state: full 2020+2021 crime_count totals per group, for each
#    of the five historical-feature groupings. Only the columns needed for
#    these aggregations are read from each prior-year file.
# ---------------------------------------------------------------------------
NEEDED_COLS = [
    "grid_id",
    "cmplnt_fr_dt",
...
prior_both = pd.concat([prior_2020, prior_2021], ignore_index=True)

carry_grid = prior_both.groupby("grid_id")["crime_count"].sum().rename("carry").reset_index()
carry_grid_hour = (
    prior_both.groupby(["grid_id", "hour"])["crime_count"].sum().rename("carry").reset_index()
)
carry_grid_day = (
    prior_both.groupby(["grid_id", "day_of_week"])["crime_count"].sum().rename("carry").reset_index()
)
carry_grid_tp = (
    prior_both.groupby(["grid_id", "time_period"], observed=True)["crime_count"]
    .sum()
    .rename("carry")
    .reset_index()
)
carry_grid_weekend = (
    prior_both.groupby(["grid_id", "is_weekend"])["crime_count"].sum().rename("carry").reset_index()
)

# Spot-check: for one grid, confirm the two ways of computing the full
# 2020+2021 total agree -- (a) sum(crime_count) over both years, vs
# (b) the last chronological 2021 row's historical_grid_crime_count + that
# row's own crime_count (which already reflects the 2020 carry baked into
# the 2021 file).
check_grid = ALL_GRIDS[0]
prior_2021_sorted = prior_2021[prior_2021["grid_id"] == check_grid].sort_values(
    ["cmplnt_fr_dt", "hour"]
)
last_row = prior_2021_sorted.iloc[-1]
...
print(
    f"[2] Spot-check grid {check_grid}: 2020+2021-sum={method_a}, "
    f"last-2021-row-historical+crime_count={method_b}, match={method_a == method_b}"
)
assert method_a == method_b, "Carry-over derivation mismatch!"

del prior_2020, prior_2021, prior_both, prior_2021_sorted
gc.collect()
track_peak()
print(
...
# ---------------------------------------------------------------------------
# 3) Load ONLY year-2022 crime rows (predicate pushdown) and aggregate to
#    (grid_id, date, hour) -> crime_count, using the same 0.01-degree grid
#    definition already used for 2020/2021.
# ---------------------------------------------------------------------------
GRID_STEP = 0.01
...
print(f"[3] Year-2022 crime rows: {n_before:,} ({n_dropped} dropped for missing lat/lon)")
...
# grid outside the reused 2020/2021 grid set).
...
print(f"[3] Aggregated (grid, date, hour) rows with >=1 crime in 2022: {len(crime_agg):,}")
...
# ---------------------------------------------------------------------------
# 4) Full 2022 calendar (365 days -- not a leap year) and hours.
# ---------------------------------------------------------------------------
all_dates_2022 = pd.date_range(f"{YEAR}-01-01", f"{YEAR}-12-31", freq="D")
all_hours = np.arange(24, dtype="int8")
print(f"[4] 2022 calendar days: {len(all_dates_2022)}")

expected_rows = N_GRIDS * len(all_dates_2022) * 24
...
#    compute leak-free WITHIN-2022 historical features, add the matching
#    2020+2021 carry-over constant, cast to compact dtypes, stream to Parquet.
# ---------------------------------------------------------------------------
writer = None
total_rows = 0
positive_rows = 0
...
for b in range(n_batches):
    ...
    g_idx, d_idx, h_idx = np.meshgrid(
        np.arange(len(batch_grids)), np.arange(len(all_dates_2022)), all_hours, indexing="ij"
    )
    chunk = pd.DataFrame(
        {
            "grid_id": batch_grids[g_idx.ravel()],
            "cmplnt_fr_dt": all_dates_2022.values[d_idx.ravel()],
    ...
    # --- calendar features (same definitions as 2020/2021) ---
    ...
    # --- leak-free historical features: WITHIN-2022 cumsum().shift(1),
    # then + the matching 2020+2021 carry-over constant for that group. ---
